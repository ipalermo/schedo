...
import com.ncode.android.apps.schedo.io.model.Expert;
...
public class ExpertsHandler extends JSONHandler {
    ...
    private static final String TAG = LogUtils.makeLogTag(ExpertsHandler.class);
    ...
    public void makeContentProviderOperations(ArrayList<ContentProviderOperation> list) {
        ...
        if (isIncrementalUpdate) {
            LogUtils.LOGD(TAG, "Doing incremental update for experts.");
        } else {
            LogUtils.LOGD(TAG, "Doing FULL (non incremental) update for experts.");
        ...
        LogUtils.LOGD(TAG, "Experts: " + (isIncrementalUpdate ? "INCREMENTAL" : "FULL") + " update. " +
    ...
    private HashMap<String, String> loadExpertHashcodes() {
        ...
        Cursor cursor = mContext.getContentResolver().query(uri, ExpertHashcodeQuery.PROJECTION,
                null, null, null);
        if (cursor == null) {
            LogUtils.LOGE(TAG, "Error querying expert hashcodes (got null cursor");
            return null;
        }
        if (cursor.getCount() < 1) {
            LogUtils.LOGE(TAG, "Error querying expert hashcodes (no records returned)");
            return null;
        }
