...
package com.ncode.android.apps.schedo.sync.userdata;
...
public abstract class AbstractUserDataSyncHelper {
    ...
    public boolean sync() {
        ...
        Cursor scheduleData = mContext.getContentResolver().query(
                ScheduleContract.MySchedule.buildMyScheduleUri(mContext, mAccountName), MyScheduleQuery.PROJECTION,
                null, null, null);
        ...
        for (UserAction action: actions) {
            ContentProviderOperation op = ContentProviderOperation.newUpdate(
                    ScheduleContract.addCallerIsSyncAdapterParameter(
                            ScheduleContract.MySchedule.buildMyScheduleUri(mContext, mAccountName)))
                    .withSelection(ScheduleContract.MySchedule.SESSION_ID + "=? AND " +
